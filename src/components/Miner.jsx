    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `https://api.helium.io/v1/hotspots/${address}/rewards/sum?min_time=-${timeFrame}%20day`,
          { mode: "cors" }
        );
    ...
    <div className="card mx-auto mt-2 mb-3 w-35 bg-primary text-white text-center">
      ...
      <div className="card-body d-flex flex-column text-center justify-content-center align-items-center">
        ...
        <p className="card-item">
          Distance:{" "}
          {isMeters ? distance.toFixed(2) : metersToMile(distance).toFixed(2)}{" "}
          {unit}
        </p>
        <p className="card-item">{geocode.short_street}</p>
        <p className="card-item">
          {geocode.short_city}, {geocode.short_state}
        </p>
        <div className="card-item mb-2" onClick={handleClick}>
          <Rewards
            {...props}
            rewards={rewards}
            timeFrame={timeFrame}
        ...
        <p
          className={
            status.online === "offline"
              ? "card-item bg-danger rounded p-1"
              : "card-item rounded p-1 bg-success"
          }
        >
          {status.online}
        </p>
